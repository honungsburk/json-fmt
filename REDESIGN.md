...
**Location:** `src/syntax.rs:40-47`

**Current Code:**
```rust
pub fn clone_subtree(&self) -> SyntaxNode {
    todo!("not implemented")  // ❌ WILL PANIC
}
...
**Options:**
1. Implement these methods properly
2. Remove them if not needed
3. Make them return `Result<SyntaxNode, Error>` if implementation is complex

**DO NOT** leave `todo!()` in public API.
...
**Problem:** `src/format.rs:32-51` directly matches on `SyntaxKind`.

**Current Code:**
```rust
fn format_value(doc: &mut Doc<'static>, value: &JsonValue) {
    match value.syntax().kind() {  // ❌ Exposes internal structure
        SyntaxKind::OBJECT => { ... }
        SyntaxKind::ARRAY => { ... }
        // ...
    }
...
**Improvement:** Add higher-level type queries to `JsonValue`:
...
**Then Formatter Becomes:**
```rust
fn format_value(doc: &mut Doc<'static>, value: &JsonValue) -> Result<(), FormatError> {
    match value.value_kind() {  // ✅ No SyntaxKind exposed
        ValueKind::Object => format_object(doc, &value.as_object().unwrap()),
        ValueKind::Array => format_array(doc, &value.as_array().unwrap()),
        ValueKind::String | ValueKind::Number | ValueKind::Boolean | ValueKind::Null => {
            format_primitive(doc, value)
        }
        ValueKind::Invalid => Err(FormatError::InvalidValueKind),
    }
}
```
...
**Problem:** `src/format.rs:128-145` manually filters trivia by kind.

**Current Code:**
```rust
fn format_token(doc: &mut Doc<'static>, token: &SyntaxToken) {
    for trivia in token.leading_trivia() {
        if trivia.kind() == SyntaxKind::COMMENT {  // ❌ Kind checking
...
**Improvement:** Add trivia accessors to `SyntaxToken`:

```rust
impl SyntaxToken {
    /// Get only comment trivia (filters out whitespace)
    ...
    /// Get all trivia (comments + whitespace)
    pub fn all_leading_trivia(&self) -> &[SyntaxToken] {
        &self.leading_trivia
    }

    pub fn all_trailing_trivia(&self) -> &[SyntaxToken] {
        &self.trailing_trivia
    }

    /// Check if token has any comments
    pub fn has_comments(&self) -> bool {
        self.leading_trivia.iter().any(|t| t.kind() == SyntaxKind::COMMENT)
            || self.trailing_trivia.iter().any(|t| t.kind() == SyntaxKind::COMMENT)
    }
}
```
...
fn format_token(doc: &mut Doc<'static>, token: &SyntaxToken) {
    ...
    for comment in token.leading_comments() {  // ✅ No kind checking
        doc.tag(comment.text().to_string());
        doc.tag(Tag::Break(1));
    }
...
### 3.3 Index-Based Access Abstraction

**Problem:** Constants like `KEY_INDEX`, `COLON_INDEX` expose positional indexing.
...
1. The grammar guarantees structure
2. Constants document the invariant
3. Access is O(1)

**But improve documentation:**
```rust
impl JsonObjectField {
    /// OBJECT_FIELD structure is guaranteed by the parser:
    /// Grammar: OBJECT_FIELD ::= STRING COLON JsonValue
    /// Children: [0: STRING token, 1: COLON token, 2: JsonValue node]
    ///
    /// This invariant is enforced in parser.rs::parse_object_field()
    const KEY_INDEX: usize = 0;
    const COLON_INDEX: usize = 1;
    const VALUE_INDEX: usize = 2;
}
